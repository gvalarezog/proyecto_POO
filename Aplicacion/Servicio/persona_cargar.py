...
from Aplicacion.Servicio.persona_modificar import PersonaModificar


class PersonaCargar(QMainWindow):
    def __init__(self):
    ...
    def actualizar(self):
        if not self.persona_escogida:
            ...
            self.vtn_hija = PersonaModificar(persona=self.persona_escogida, vtn_padre=self)
            self.vtn_hija.show()

    def nuevo(self):
        self.vtn_hija = PersonaModificar(vtn_padre=self)
        self.vtn_hija.show()
